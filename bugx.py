...
class Bug0(Node):
    def __init__(self):
        ...
        self.start_x = None
        self.start_y = None
        self.naredba_brzina_poruka = Twist()
    ...
    def bug_algorithm_callback(self):
        ...
        self.calc_y = ((self.goal_y-self.start_y)/(self.goal_x-self.start_x)) * (self.current_x-self.start_x) + self.start_y

        self.angle_diff = self.normalize_angle(goal_angle - self.current_theta)

        if goal_distance < 0.1:
            self.naredba_brzina_poruka.linear.x = 0.0
            self.naredba_brzina_poruka.angular.z = 0.0
            self.cmd_pub.publish(self.naredba_brzina_poruka)
            print("Goal reached!")
            self.state = 'MOVE_TO_GOAL'
        ...
        if self.state == 'MOVE_TO_GOAL':
            ...
            if abs(self.calc_y - self.current_y) < 0.1 and (self.b > 200 or self.c == 1):
                self.state = 'MOVE_TO_GOAL'
            else:
            ...
            if self.fl_sensor_value >= 0.9:    
                self.naredba_brzina_poruka.linear.x = 0.45
                self.naredba_brzina_poruka.angular.z = 0.9
            if self.fl_sensor_value < 0.9:
                self.naredba_brzina_poruka.linear.x = 0.0    
                self.naredba_brzina_poruka.angular.z = 0.0
    ...
    def move_to_goal(self, goal_angle):
        if abs(self.angle_diff) > 0.1:
            ...
            if self.angle_diff > 0:
                self.naredba_brzina_poruka.angular.z = 0.25
            else:
                self.naredba_brzina_poruka.angular.z = -0.25
        else: 
                self.naredba_brzina_poruka.linear.x = 0.25
                self.naredba_brzina_poruka.angular.z = 0.0
                self.b = 0
        self.cmd_pub.publish(self.naredba_brzina_poruka)
    ...
    def follow_obstacle(self):
            ...
            if self.fl_sensor_value > self.obstacle_threshold and self.fr_sensor_value < 0.8:    
                self.naredba_brzina_poruka.linear.x = 0.05    
                self.naredba_brzina_poruka.angular.z = -0.55
            ...
            if self.fl_sensor_value > 0.45 and self.fr_sensor_value > 0.8:
                self.naredba_brzina_poruka.linear.x = 0.1
                self.naredba_brzina_poruka.angular.z = 0.25
